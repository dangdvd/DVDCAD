# DVDCAD — PWA xem bản vẽ DWG/DXF (cài được, chạy offline)
...
1. Kéo–thả cả thư mục này vào. Netlify cho ngay một địa chỉ HTTPS để mở.
...
1. Bấm nút **Chia sẻ** → **Thêm vào MH chính** (Add to Home Screen). Giờ có icon app riêng, mở toàn màn hình.
1. **Offline:** ngay lần mở đầu, app đã được lưu cache; sau đó tắt mạng vẫn mở và xem **DXF** được.
1. **DWG offline:** bộ giải mã DWG nặng ~15MB nên chỉ tải khi bạn **mở một file .dwg lần đầu (cần mạng)**.
